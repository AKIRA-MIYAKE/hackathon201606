...
var timerId;
var intervalId;
var timelimitMs = 300000; //自動で録音停止するまでの時間
...
function typeResultText(appendText) {
    ...
    var recognizedText = $('#js-text-textview').val();
    if (recognizedText) {
        var $list = $('#js-dirty-word-list');
        $.get('/api/dirty-word/check/' + recognizedText)
        .done(function(words) {
          if (words.length > 0) {
            $list.empty();
            var itemsString = '';
            $.each(words, function(index, word) {
              itemsString = itemsString + '<li>' + word.value  + '</li>';
            });
            $list.append($(itemsString));
          }
        });
    }
}
